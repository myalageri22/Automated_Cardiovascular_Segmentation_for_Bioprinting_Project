...
def load_saved_split(
    ...
    """Load split data from an existing JSON file."""
    ...
    train_cases = data.get("train", [])
    val_cases = data.get("val", [])
    test_cases = data.get("test", [])
    
    logger.info(
        f"Loaded split from {split_path}: train={len(train_cases)}, "
...
def main():
    ...
    parser.add_argument("--resplit", action="store_true", help="Ignore existing splits.json and create a new split")
    ...
    parser.add_argument("--eval_split", type=str, default="test", choices=["val", "test"], help="Split to evaluate in eval_only mode")
    ...
    if split_path.exists() and not args.resplit and not args.use_official_split:
        train_cases, val_cases, test_cases = load_saved_split(split_path, logger)
    else:
    ...
    if args.eval_only:
        eval_cases = test_cases if args.eval_split == "test" else val_cases
        if not eval_cases:
            logger.error(
                f"No {args.eval_split} cases available. "
                f"Create a split with --test_ratio>0 or use --use_official_split."
            )
            sys.exit(1)
        
        logger.info(f"Creating {args.eval_split} dataset (cache_rate={config.cache_rate_val})...")
    ...
    if args.eval_only and not args.resume:
        default_ckpt = config.checkpoint_dir / "checkpoint_best.pt"
        if default_ckpt.exists():
            args.resume = str(default_ckpt)
        else:
            logger.error("eval_only requires --resume or checkpoint_best.pt in checkpoint_dir")
            sys.exit(1)
    ...
    if args.eval_only:
        logger.info(f"Running evaluation on {args.eval_split} split")
        trainer = Trainer(config, logger)
        eval_loss, eval_metrics = trainer.validate(
            model, eval_loader, loss_fn, metrics, epoch=0
        )
        
        result = {
            "split": args.eval_split,
            "checkpoint": str(ckpt_path) if ckpt_path else None,
            "num_cases": len(eval_cases) if eval_cases is not None else 0,
            "loss": eval_loss,
            "metrics": eval_metrics,
        }
        result_path = config.checkpoint_dir / f"{args.eval_split}_metrics.json"
        with open(result_path, "w") as f:
            json.dump(result, f, indent=2)
        
        logger.info(f"{args.eval_split} loss: {eval_loss:.4f}")
        for k, v in eval_metrics.items():
            if isinstance(v, (float, int)):
                logger.info(f"{args.eval_split} {k}: {v:.4f}")
        logger.info(f"Saved {args.eval_split} metrics to {result_path}")
        return
